    ...
    def __init__(self : Self) -> Literal[None]:
        ...
        def setBySystemTheme() -> Literal[None]:
            if (darkdetect.isLight()):
                ...
                self.btnRU.configure(bg_color=Materials.Colors.white)
            ...
            if (darkdetect.isDark()):
                ...
                self.btnRU.configure(bg_color=Materials.Colors.dark)
        ...
        def changeAppLanguage(arg : str) -> Literal[None]:
            if (arg == 'RU'):
                self.root.title(string='Приложение To Do')
                self.tabControl.add(child=self.tabTodo, text='Todo')
                self.tabControl.add(child=self.tabLang, text='Язык')
                self.tabControl.add(child=self.tabAbout, text='O приложении')
                self.btnAdd.configure(text='Добавить')
                self.btnDelete.configure(text='Удалить')
                self.btnRU.configure(text='Русский')
                self.btnEN.configure(text='Английский')
                self.btnGithub.configure(text='ГитХаб')
            if (arg == 'EN'):
                self.root.title(string='Todo App')
                self.tabControl.add(child=self.tabTodo, text='Todo')
                self.tabControl.add(child=self.tabLang, text='Language')
        ...
        self.btnRU = CTkButton(
            ...
            height=50,
            fg_color=Materials.Colors.purple,
            command=lambda:changeAppLanguage(arg='RU'),
        )
        
        self.btnRU.place(relx=0.25, rely=0.5, anchor=Materials.Alignments.center)
